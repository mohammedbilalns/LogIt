...
import {
  ...
  fetchLog,
  clearCurrentLog,
} from '@/infrastructure/store/slices/logSlice';
...
import type { CreateLogResult, LogLimitExceededResponse } from '@/domain/usecases/log/createLog';
    ...
    try {
      if (mode === 'create') {
        const result = await dispatch(createLog({
          ...values,
        })).unwrap() as CreateLogResult;
        if ('limitExceeded' in result && result.limitExceeded) {
          if (onSubscriptionLimitError) {
            onSubscriptionLimitError(result as LogLimitExceededResponse);
          }
          return;
        }
        notifications.show({
