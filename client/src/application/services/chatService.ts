...
export const chatService = {
  async fetchUserChats({ page = 1, limit = 10 }: { page?: number; limit?: number }) {
    const response = await axiosInstance.get(
      `${API_ROUTES.CHATS.BASE}?page=${page}&limit=${limit}`
    );
  ...
  async fetchChatDetails({
    ...
    const response = await axiosInstance.get(
      `${API_ROUTES.CHATS.BY_ID(chatId)}?page=${page}&limit=${limit}`
    );
    return { ...response.data, page };
  },
  ...
  async fetchUserGroupChats({ page = 1, limit = 10 }: { page?: number; limit?: number }) {
    const response = await axiosInstance.get(
      `${API_ROUTES.CHATS.GROUP}?page=${page}&limit=${limit}`
    );
    return { ...response.data, page };
  },
  ...
  async addParticipants({ chatId, participants }: { chatId: string; participants: string[] }) {
    const response = await axiosInstance.post(API_ROUTES.CHATS.PARTICIPANTS(chatId), {
      participants,
    });
