...
export class ChatService implements IChatService {
  ...
  async createChat(creatorId: string, data: CreateChatDto) {
    ...
    if (!isGroup) {
      ...
      const existingChat = await this.chatRepository.findPrivateChat(allParticipants[0], allParticipants[1]);
  ...
  async addParticipant(chatId: string, userId: string, requesterId: string) {
    const chat = await this.chatRepository.findById(chatId);
    if (!chat || !chat.isGroup) throw new BadRequestError(HttpResponse.CHAT_NOT_FOUND_OR_NOT_GROUP);

    const requester = await this.chatParticipantRepository.findParticipant(chatId, requesterId);
    if (!requester || requester.role !== 'admin') {
      throw new UnauthorizedError(HttpResponse.NO_PERMISSION_ADD);
    }
    
    const existingParticipant = await this.chatParticipantRepository.findParticipant(chatId, userId);
  ...
  async sendMessage(chatId: string, senderId: string, data: SendMessageDto) {
    const participant = await this.chatParticipantRepository.findParticipant(chatId, senderId);
  ...
  async getOrCreatePrivateChat(userId: string, targetUserId: string): Promise<Chat> {
    logger.yellow('CHAT_DEBUG', `getOrCreatePrivateChat called with userId=${userId}, targetUserId=${targetUserId}`);
