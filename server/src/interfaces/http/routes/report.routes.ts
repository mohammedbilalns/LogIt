...
router.use(asyncHandler((req, res, next) => authorizeRoles('admin', 'superadmin')(req, res, next)));

// Get all reports 
router.get('/', asyncHandler((req, res) => reportController.getReports(req, res)));

// Update report status
router.patch('/:id/status', asyncHandler((req, res) => reportController.updateReportStatus(req, res)));

// Block article
router.post('/block-article/:articleId', asyncHandler((req, res) => reportController.blockArticle(req, res)));
